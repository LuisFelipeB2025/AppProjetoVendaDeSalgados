...
import { View, FlatList, StyleSheet, Image, StatusBar,TouchableOpacity,Text,Modal,TouchableWithoutFeedback,Alert,Platform,TextInput,Linking, ImageBackground } from 'react-native';
...
export default function HomeScreen({ onLogout, user, onNavigateToProfile }: Props) {
  ...
  const handleFinalizarCompra = () => {
    ...
    mensagem += `${bairro}\n`;
    if (user?.cep) mensagem += `📮 CEP: ${user.cep}\n`;
    mensagem += `\n*ITENS DO PEDIDO:*\n`;
    ...
    <ImageBackground source={BACKGROUND_IMAGE} style={styles.backgroundImage} resizeMode="cover">
      ...
      <View style={[styles.overlay, { paddingTop: insets.top }]}>
        ...
        <View style={styles.header}>
          ...
          <Image source={require('../assets/LGT.png')} style={styles.logo} />
...
const styles = StyleSheet.create({
  ...
  logo: { width: 250, height: 90, resizeMode: 'contain' },
  ...
  emptyText: { textAlign: 'center', marginTop: 40, color: '#fff', fontWeight: '500', fontSize: 18 },
  ...
  checkoutLabel: { fontSize: 12, color: '#666' },
  checkoutValue: { fontSize: 18, fontWeight: 'bold', color: '#333' },
  checkoutButton: {
    ...
    borderRadius: 30,
    elevation: 2,
    ...Platform.select({ web: { cursor: 'pointer', transition: '0.2s' } as any })
  },
  checkoutButtonDisabled: { backgroundColor: '#e0e0e0' },
